...
export function yachtzeeChainDiet(simOnly?: boolean): boolean {
	...
	const fullnessAvailable =
		myLevel() >= 13
			? Math.max(
					0,
					fullnessLimit() - myFullness() + toInt(haveDistentionPill) - reservedFullness
			  )
			: 0;
	...
	const canParkaChain =
		fishyPipeTurnsAvailable + (fishyPipeTurnsAvailable > 0 && pyecAvailable() ? 5 : 0) >=
		freeNCs();
	...
	const extroValuePerSpleen = (4 * MPE - 4 * VOA - extroPrice) / 2;
	...
	if (haveEffect($effect`Fishy`) + 5 * toInt(havePYECCharge) < yachtzeeTurns) {
		throw new Error(
			`We only got ${haveEffect($effect`Fishy`)}/${yachtzeeTurns} turns of fishy!`
		);
	}
