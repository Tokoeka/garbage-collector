...
export const chainStarters = [
	...
	new WitchessFight(
		...
		(options: WitchessFightRunOptions) => {
			withMacro(
				options.macro,
				() => CombatLoversLocket.reminisce(copyTarget),
				options.useAuto
			);
		}
	),
		...
		() => {
			...
			if (havePutty && haveRainDoh) {
				if (puttyLocked && rainDohLocked) return 0;
					...
					return (
						5 - get("spookyPuttyCopiesMade") + itemAmount($item`Spooky Putty monster`)
					);
				}
				return (
					6 -
				...
				return (
					5 - get("_raindohCopiesMade") + itemAmount($item`Rain-Doh box full of monster`)
				);
			}
			return 0;
		},
...
export const wanderSources = [
	new WitchessFight(
		"Digitize",
		() =>
			get("_sourceTerminalDigitizeMonster") === copyTarget &&
...
export const conditionalSources = [
	...
	new WitchessFight(
		...
		(options: WitchessFightRunOptions) => {
			adventureMacro(
				$location`The Dire Warren`,
				Macro.if_(copyTarget, options.macro).abort()
			);
		},
		{
			requirements: [
