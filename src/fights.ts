		...
		withStash($items`Spooky Putty sheet`, () => {
			...
			if (have($familiar`Pocket Professor`)) {
				const potentialPocketProfessorLectures = [
					...
					},
					{
						...
						goalMaximize: (requirements: Requirement) =>
							maximizeCached(
								requirements.maximizeParameters,
								requirements.maximizeOptions
							),
					},
				];
				...
				for (const potentialLecture of potentialPocketProfessorLectures) {
					...
					goalMaximize(
						Requirement.merge([professorRequirement, ...fightSource.requirements])
					);
...
class FreeFight {
	...
	runAll() {
		...
		if ((this.options.cost ? this.options.cost() : 0) > get("garbo_valueOfFreeFight", 2000)) {
			return;
		}
		while (this.isAvailable()) {
			...
			freeFightOutfit(
				this.options.requirements
					? Requirement.merge(this.options.requirements())
					: undefined
			);
			safeRestore();
...
class FreeRunFight extends FreeFight {
	...
	runAll() {
		...
		while (this.isAvailable()) {
			...
			if (!runSource) break;
			useFamiliar(
				runSource.constraints.familiar?.() ??
...
const freeFightSources = [
	new FreeFight(
		...
		{
			requirements: () => [
				new Requirement([], { forceEquip: $items`protonic accelerator pack` }),
			],
		}
	),
	new FreeFight(
	...
	new FreeFight(
		...
		false,
		{
			...
			familiar: () =>
				have($familiar`Red-Nosed Snapper`) ? $familiar`Red-Nosed Snapper` : null,
...
const freeKillSources = [
	...
	new FreeFight(
		...
		true,
		{
			...
			requirements: () => [
				sandwormRequirement().merge(
					new Requirement([], { forceEquip: $items`Jurassic Parka` })
				),
			],
			effects: () =>
				have($skill`Emotionally Chipped`) && get("_feelLostUsed") < 3
...
function setNepQuestChoicesAndPrepItems() {
	...
	/* if (quest === "food") {
		...
		if (!questStep("_questPartyFair")) {
			...
			const partyFairInfo = get("_questPartyFairProgress").split(" ");
			logMessage(
				`Gerald wants ${partyFairInfo[0]} ${toItem(partyFairInfo[1]).plural}, please!`
			);
		}
	} else {
...
const haveEnoughPills =
	clamp(availableAmount($item`synthetic dog hair pill`), 0, 100) +
		...
		200 &&
	availableAmount($item`Map to Safety Shelter Grimace Prime`) < ESTIMATED_OVERDRUNK_TURNS;
function wantPills(): boolean {
	return (
		have($item`Fourth of May Cosplay Saber`) && crateStrategy() !== "Saber" && haveEnoughPills
	);
}
