...
import { copyTargetCount, copyTargetSources, getNextCopyTargetFight } from "./embezzler";
import {
  ...
  if (digitizeNeedsStarting || romanceNeedsStarting) {
    ...
    do {
      ...
      garboAdventure(
        ...
        Macro.if_(globalOptions.target, Macro.embezzler("wanderer")).step(run.macro),
      );
    } while (
    ...
    withStash($items`Spooky Putty sheet`, () => {
      ...
      while (nextFight !== null && myAdventures()) {
        ...
        if (
          ...
          (familiarEquippedEquipment($familiar`Obtuse Angel`) === $item`quake of arrows` ||
        ...
        if (
          ...
          (nextFight.wrongEncounterName || get("lastEncounter") === globalOptions.target.name)
        ) {
          eventLog.initialCopyTargetsFought++;
...
function latteFight(
  ...
  return new FreeRunFight(
    ...
    (runSource: ActionSource) => {
      ...
      propertyManager.setChoices(wanderer().unsupportedChoices.get(location) ?? {});
    ...
    () => {
      garboAdventure(
        ...
        Macro.step("pickpocket")
          ...
          .ifHolidayWanderer(Macro.basicCombat())
          .externalIf(get("_VYKEACompanionLevel") > 0, Macro.trySkill($skill`Summon Mayfly Swarm`))
...
const freeKillSources = [
  ...
  new FreeFight(
    () => !get("_missileLauncherUsed") && getWorkshed() === $item`Asdon Martin keyfob (on ring)`,
    () => {
      ensureBeachAccess();
