...
function menu(): MenuItem<Note>[] {
  ...
  return [
    ...
    new MenuItem($item`tin cup of mulligan stew`, { maximum: 1 }),
    ...
    new MenuItem($item`Hodgman's blanket`, { maximum: 1 }),
        ...
        balanceMenu(
          menu().filter(
            (menuItem) =>
              ...
              [Mayo.flex, Mayo.zapine, $item`Special Seasoning`, $item`whet stone`].includes(
                menuItem.item,
              ),
          ),
          pantsgivingDietPlanner,
