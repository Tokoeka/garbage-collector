...
import { getPrioritizedStations, grabMedicine, rotateToOptimalCycle } from "../../resources";
...
const worksheds = [
  ...
  new GarboWorkshed({
    ...
    available: () =>
      get("_nextColdMedicineConsult") <= totalTurnsPlayed() && myTotalTurnsSpent() !== _lastCMCTurn, // TODO: Ensure that we have a good expected cmc result
    action: () => {
      grabMedicine();
  ...
  return [
    ...worksheds.map(workshedTask),
    {
      ...
      ready: () => {
        ...
        const haveNext = GarboWorkshed.next !== null && have(GarboWorkshed.next.workshed);
        const enoughTurns =
          !GarboWorkshed.next?.minTurns ||
          GarboWorkshed.next.minTurns + SAFETY_TURNS_THRESHOLD > estimatedGarboTurns();
