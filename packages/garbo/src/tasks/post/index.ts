...
function floristFriars(): GarboPostTask {
  return {
    ...
    do: () =>
      BARF_PLANTS.filter((flower) => flower.available()).forEach((flower) => flower.plant()),
    available: () => FloristFriar.have() && BARF_PLANTS.some((flower) => flower.available()),
  };
}
...
function refillCinch(): GarboPostTask {
  return {
    ...
    available: () => CinchoDeMayo.have() && totalFreeRests() > get("timesRested"),
  };
}
...
function eightBitFatLoot(): GarboPostTask {
  return {
    ...
    ready: () => canAdventure($location`The Spooky Forest`) && get("8BitScore") >= 20_000,
    do: () => {
