...
const FreeFightTasks: GarboFreeFightTask[] = [
  ...
  },
  {
    ...
    name: "August 8th",
    ready: () => (shouldAugustCast($skill`Aug. 8th: Cat Day!`) ? true : false),
    completed: () => get("_aug8Cast"),
  ...
  },
  {
    ...DEFAULT_FREE_FIGHT_TASK,
    name: "August 22nd",
    ready: () => (shouldAugustCast($skill`Aug. 22nd: Tooth Fairy Day!`) ? true : false),
    completed: () => get("_aug22Cast"),
  ...
  },
  {
    ...
    choices: () => wanderer().getChoices({ wanderer: "wanderer", allowEquipment: false }),
    combat: new GarboStrategy(() => Macro.basicCombat()),
...
export function expectedFreeFights(): number {
  ...
  return sum(availableFights, ({ combatCount, tentacle }) => combatCount() * (tentacle ? 2 : 1));
}
...
export function possibleTentacleFights(): number {
  ...
  return sum(availableFights, ({ combatCount, tentacle }) => combatCount() * (tentacle ? 1 : 0));
}
