...
import { augustSummonTasks, candyMapDailyTasks, doingGregFight } from "../resources";
...
const DailyItemTasks: GarboTask[] = [
  ...
  },
  {
    ...
    ready: () =>
      have($item`Clan VIP Lounge key`) && getClanLounge()["Olympic-sized Clan crate"] !== undefined,
  ...
  },
  {
    ...
    outfit: () => ({
      ...
      offhand: myInebriety() > inebrietyLimit() ? $item`Drunkula's wineglass` : undefined,
    }),
    limit: { skip: 3 },
    spendsTurn: false,
  },
  {
    ...
    do: (): void => {
      ...
      _shouldClearRufusQuest = value.some(
        (value) => (!globalOptions.nobarf || value.name === "embezzler") && value.value - price > 0,
      );
