...
function wanderTask(
  ...
  return {
    ...
    outfit: () => freeFightOutfit(undelay(spec), { wanderOptions: undelay(details) }),
    spendsTurn: false,
...
function dailyDungeon(additionalReady: () => boolean) {
  return {
    ...
    acquire: $items`ring of Detect Boring Doors, eleven-foot pole, Pick-O-Matic lockpicks`.map(
      (i) => ({ item: i }),
    ),
...
function gingerbreadMidnight(additionalReady: () => boolean) {
  return {
    ...
    outfit: () => ({
      equip:
        bestMidnightAvailable().location === $location`Gingerbread Upscale Retail District`
...
const NonBarfTurnTasks: AlternateTask[] = [
  ...
  {
    name: "Map for Pills",
    completed: () => availableAmount($item`Map to Safety Shelter Grimace Prime`) === 0,
    choices: () => ({
      536:
...
const BarfTurnTasks: GarboTask[] = [
  ...
  },
  {
    ...
    outfit: () =>
      digitizedEmbezzler()
        ...
        : freeFightOutfit(),
    do: () => wanderer().getTarget({ wanderer: "wanderer", allowEquipment: false }),
    choices: () =>
      wanderer().getChoices({
        wanderer: "wanderer",
        allowEquipment: false,
  ...
  {
    name: "Envyfish Egg",
    ready: () => have($item`envyfish egg`) && get("envyfishMonster") === globalOptions.target,
  ...
  ),
  {
    ...
    choices: { 1204: 1 },
    combat: new GarboStrategy(() =>
      Macro.abortWithMsg("We thought it was noon here in Gingerbread City, but we're in a fight!"),
    ),
...
function nonBarfTurns(): number {
  ...
  return sum(
    NonBarfTurnTasks.filter((t) => (undelay(t.sobriety) ?? sobriety) === sobriety),
    (t) => undelay(t.turns),
  );
}
...
export const BarfTurnQuests = [TurnGenQuest, WandererQuest, NonBarfTurnQuest, BarfTurnQuest];
