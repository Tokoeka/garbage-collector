...
import { attemptCompletingBarfQuest, checkBarfQuest, checkVolcanoQuest } from "../resources";
...
function pantogram(): void {
  ...
  if (have($item`repaid diaper`) && have($familiar`Robortender`)) {
    const expectedBarfTurns = globalOptions.nobarf
      ? 0
      : estimatedGarboTurns() - digitizedMonstersRemaining() - copyTargetCount();
...
const DailyTasks: GarboTask[] = [
  ...
  {
    name: "Free Goofballs",
    ready: () => canAdventure($location`The Spooky Forest`) && get("questL03Rat") !== "unstarted",
    completed: () => get("lastGoofballBuy") === myAscensions(),
    after: ["Daily/Unlock Woods"],
    do: () => {
      if (itemAmount($item`gloomy black mushroom`) > 0) {
        putCloset(itemAmount($item`gloomy black mushroom`), $item`gloomy black mushroom`);
      }
      visitUrl("woods.php"); // Without visiting woods, other visitUrls will not register woods as being unlocked.
  ...
  },
  {
    ...
    ready: () =>
      ...
      isOnline("OnlyFax") &&
      Clan.getWhitelisted().find((c) => c.name === "Bonus Adventures from Hell") !== undefined,
