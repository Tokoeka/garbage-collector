...
export function safeInterrupt(): void {
  if (globalOptions.prefs.rolloverBuffer * 60 * 1000 > rollover() * 1000 - Date.now()) {
    throw new Error(
      `Eep! It's a mere ${Math.round(rollover() - Date.now() / 1000)} seconds until rollover!`,
    );
  }
...
export function checkGithubVersion(): void {
  if (process.env.GITHUB_REPOSITORY === "CustomBuild") {
    ...
    const releaseSHA = gitBranches.find((branchInfo) => branchInfo.name === "release")?.commit?.sha;
    ...
    print("Garbo was built from an unknown repository, unable to check for update.", HIGHLIGHT);
  }
}
...
export function lastAdventureWasWeird(options: Partial<LastAdventureOptions> = {}): boolean {
  const { extraEncounters, includeGhostDog, includeHolidayWanderers, includeJuneCleaver } = {
    ...DEFAULT_LAST_ADVENTURE_OPTIONS,
    ...options,
  };
  return [
    ...extraEncounters,
    ...(includeGhostDog ? GHOST_DOG_ADVENTURES : []),
    ...(includeHolidayWanderers ? getTodaysHolidayWanderers().map((monster) => monster.name) : []),
...
function getDropsList(key: keyof GarboItemLists) {
  return (listItems[key] ??= (JSON.parse(fileToBuffer("garbo_item_lists.json")) as GarboItemLists)[
    key
  ].map((i) => Item.get(i)));
}
...
export function getCombatFlags(...flags: string[]): { flag: string; value: boolean }[] {
