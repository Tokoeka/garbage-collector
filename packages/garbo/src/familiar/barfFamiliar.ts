...
export function barfFamiliar(): { familiar: Familiar; extraValue: number } {
  ...
  if (viableMenu.every(({ limit }) => limit !== "none")) {
    const turnsNeeded = sum(viableMenu, turnsNeededFromBaseline(meatFamiliarEntry));
  ...
  const familiarPrintout = ({ expectedValue, familiar, outfitValue }: MarginalFamiliar) =>
    `(expected value of ${expectedValue.toFixed(1)} from familiar drops, ${familiarAbilityValue(
      familiar,
    ).toFixed(1)} from familiar abilities and ${outfitValue.toFixed(1)} from outfit)`;
