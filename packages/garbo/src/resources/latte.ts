...
import { $item, $skill, byStat, get, have, Latte, setEqual, Tuple } from "libram";

type Ingredients = Tuple<Latte.Ingredient, 3>;
function desirableIngredients() {
  return have($skill`Head in the Game`) && have($item`mafia pointer finger ring`)
...
export function shouldUnlockIngredients(): boolean {
  ...
  const shouldTryToUnlockIngredients =
    desirableIngredients().filter(
      (i) => Latte.ingredientsUnlocked().includes(i) || canAdventure(Latte.locationOf(i)),
    ).length >= 3;
  const doneUnlockingIngredients =
    desirableIngredients().filter((i) => Latte.ingredientsUnlocked().includes(i)).length >= 3;
  return shouldTryToUnlockIngredients && !doneUnlockingIngredients;
}

function ingredientsToFillWith(): Ingredients {
  return [
    ...desirableIngredients().filter((i) => Latte.ingredientsUnlocked().includes(i)),
...
export function shouldFillLatte(): boolean {
  if (!have($item`latte lovers member's mug`) || get("_latteRefillsUsed") >= 3) {
    return false;
  }
