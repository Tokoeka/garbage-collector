...
export function getTreatOutfit(): string {
  ...
  if (!availableOutfits.length) {
    print("You don't seem to actually have any trick-or-treating outfits available, my friend!");
  }
...
function doCandyTreat(): GarboTask {
  return {
    name: "Treat",
    completed: () => treated,
    ready: () => !holiday().includes("Halloween") && get("_mapToACandyRichBlockUsed"),
    outfit: treatOutfit,
    do: (): void => {
    ...
    combat: new GarboStrategy(() =>
      Macro.abortWithMsg("We were planning on Treating, but we've been Tricked!"),
    ),
  };
}
