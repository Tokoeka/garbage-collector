...
const isConned = (monster: Monster): boolean => get("longConMonster") === monster;
const isTurtlesexed = (monster: Monster): boolean => get("_gallapagosMonster") === monster;
const isLatted = (monster: Monster): boolean => Latte.sniffedMonster() === monster;
export function crateStrategy(): "Sniff" | "Saber" | "Orb" | null {
  if (!doingGregFight()) return null;
  if (
    (have($skill`Transcendent Olfaction`) && (isOlfacted(crate) || get("_olfactionsUsed") < 2)) ||
...
function initializeCrates(): void {
  do {
    ...
    if (
      ...
      const sniffrun = Macro.trySkill($skill`Transcendent Olfaction`)
        ...
        .externalIf(!isTurtlesexed(crate), Macro.trySkill($skill`Gallapagosian Mating Call`))
  ...
  } while (!["crate", "Using the Force"].includes(get("lastEncounter")) && !isBanished(crate)); // loop until we actually hit a crate
...
function banishBunny(): void {
  ...
  do {
    ...
    garboAdventure(
      ...
      Macro.if_($monster`fluffy bunny`, banish.macro()).embezzler("fluffy bunny banish"),
    );
  } while (
...
export function gregReady(): boolean {
  return (
    (get("beGregariousMonster") === globalOptions.target && get("beGregariousFightsLeft") > 0) ||
...
export function possibleGregCrystalBall(): number {
  if (have($item`miniature crystal ball`)) {
    const ponderCount =
      CrystalBall.ponder().get($location`The Dire Warren`) === globalOptions.target ? 1 : 0;
